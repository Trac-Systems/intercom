...
test('main peer starts MSB with its persisted wallet', () => {
  assert.match(mainSource, /const loadPeerWallet = async \(config\) => \{/);
  assert.match(mainSource, /await wallet\.importFromFile\(config\.keyPairPath, b4a\.alloc\(0\)\);/);
  assert.match(mainSource, /const msbWallet = await loadPeerWallet\(msbConfig\);/);
